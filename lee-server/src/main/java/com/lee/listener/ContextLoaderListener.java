...
public class ContextLoaderListener implements ServletContextListener{
  ...
  public void contextInitialized(ServletContextEvent sce) {
    // 웹 애플리케이션이 시작될 때 서블릿 컨테이너가 호출하는 메소드
    ...
    if (contextClass != null) {
      try {
        // web.xml에 설정한 IoC 컨테이너의 클래스를 로딩한다.
      ...
      // 이 애노테이션이 붙은 클래스가 Java Config 클래스 이다.
      // IoC 컨테이너는 이 Java Config에 설정된 대로 객체를 준비한다.
      appCtx = new AnnotationConfigApplicationContext("com.lee.config");
    }
