...
public class BufferedInputStream extends FileInputStream {
  ...
  public int read() throws IOException {
    if (cursor >= size) { // 버퍼에 보관된 데이터를 다 읽었으면,
      count++;
      size = read(buf); // 다시 상속받은 메소드를 사용하여 1024 바이트를 읽어 온다.
      if (size == -1) // 물론 읽은 데이터가 없다면, 즉, 파일의 데이터를 다 읽었다면 -1을 리턴한다.
        return -1;
      cursor = 0; // FileInputStream을 사용하여 새로 1024 바이틀르 읽어 버퍼에 저장했다면,
                  // 다시 커서의 위치를 0으로 설정한다.
    }
    
    // 배열에 들어 있는 바이트의 값이 양수일 경우 int로 리턴하면 그 값 그대로 리턴된다.
    // 예) 0x7f(01001111, 10진수로 127) 값을 리턴한다면 4바이트 int값 0x0000007f 가 리턴된다.
    // 문제는 첫 비트가 1로 시작하는 바이트 값을 int로 리턴하는 경우이다.
    // JVM은 첫 비트가 1일 경우 음수로 간주하고 byte값을 int로 바꿀 때 앞의 3바이트를 모두 음수를 의미하는 1로 설정한다.
    // 예) 0x80(10000000, 10진수로 128) 값을 리턴한다면 4바이트 int값 0xffffff80 이 리턴된다.
