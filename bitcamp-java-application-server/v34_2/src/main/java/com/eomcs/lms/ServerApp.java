...
public class ServerApp {
  ...
  public void execute() {
    ...
    try (ServerSocket serverSocket = new ServerSocket(this.port)) {
      ...
      while (true) {
        ...
        // 단, RequestHandler의 작업을 별도의 실행 흐름으로 분리하여 실행시킨다.
        ...
        // 스레드를 분리하여 실행시키면 즉시 main thread는 즉시 다음 명령을 실행한다.

        // 클라이언트 중에 하나가 서버 종료를 설정했다면, 현재 접속한 클라이언트 요청까지만 처리하고
        // 서버 실행을 멈춘다.
        // 주의!
        // main() 메소드 호출이 종료되었다 하더라도 실행 중인 스레드가 있으면
        // JVM이 완전히 종료되지 않는다.
  ...
  private class RequestHandler extends Thread {
    ...
    // 별도의 실행 흐름(실; 스레드)은 run() 메소드에 작성한다.
    @Override
    public void run() {
