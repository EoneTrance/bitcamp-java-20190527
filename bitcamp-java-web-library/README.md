...
- /META-INF/services/java.servlet.ServletContainerInitalizer 파일 생성
- 해당 파일에 구현체 클래스의 이름을 등록한다.
- 'gradle jar'를 실행하여 .jar 파일을 생성한다.
- .jar 파일을 웹 애플리케이션 프로젝트에 import한 다음에 서블릿 컨테이너를 실행하라.
- 콘솔창에 onStartup() 메소드가 호출된 것을 확인할 수 있을 것이다.
...
- ServletContainerInitializerImpl 변경
  - MyWebInitializer 구현체의 객체를 생성한 후 지정된 메소드를 호출한다.
