...
public class HeaderServlet extends HttpServlet{
  ...
  protected void service(HttpServletRequest request, HttpServletResponse response)
    ...
    // 인클루딩 되는 서블릿에서는 setContextType()을 호출해봐야 소용없다.
    // 이 서블릿을 요청하는 쪽에서 처리해야 한다.
    //response.setContentType("text/html;charset=UTF-8");

    PrintWriter out =response.getWriter();
    out.println("<nav id='header' class='navbar navbar-expand-lg navbar-dark bg-primary'>");
    ...
    if (loginUser == null) {
      ...
      out.printf("<a href='/member/detail?no=%d' style='color:white;'>%s</a>",
          loginUser.getNo(),
          loginUser.getName());
