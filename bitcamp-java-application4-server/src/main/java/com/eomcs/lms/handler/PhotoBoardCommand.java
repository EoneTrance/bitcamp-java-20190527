...
public class PhotoBoardCommand {
  ...
  @RequestMapping("/photoboard/add") // 클라이언트 요청이 들어왔을 때 이 메소드를 호출하라고 표시한다.
  ...
  @RequestMapping("/photoboard/delete") // 클라이언트 요청이 들어왔을 때 이 메소드를 호출하라고 표시한다.
  ...
  @RequestMapping("/photoboard/detail") // 클라이언트 요청이 들어왔을 때 이 메소드를 호출하라고 표시한다.
  ...
  @RequestMapping("/photoboard/list") // 클라이언트 요청이 들어왔을 때 이 메소드를 호출하라고 표시한다.
  ...
  @RequestMapping("/photoboard/update") // 클라이언트 요청이 들어왔을 때 이 메소드를 호출하라고 표시한다.
  public void update(BufferedReader in, PrintStream out) {
    try {
      ...
      // 파일을 변경할 지 여부를 묻는다.
      out.println("사진은 일부만 변경할 수 있습니다.");
      ...
      try {txManager.rollback();} catch (Exception e1) {e1.printStackTrace();}
