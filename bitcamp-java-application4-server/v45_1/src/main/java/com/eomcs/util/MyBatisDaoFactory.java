...
public class MyBatisDaoFactory {
  ...
  public <T> T createDao(Class<T> clazz) {
    return (T) Proxy.newProxyInstance(
        ...
        new Class[] {clazz},
        // InvocationHandler 구현체의 람다 메소드
        (Object proxy, Method method, Object[] args) -> {
          // 자동으로 생성된 DAO 구현체에 대해 메소드를 호출하면 최종적으로 이 메소드가 호출된다.
          ...
          try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
            // select/insert/update/delete를 구분하여 메소드를 호출
            // 어떻게? 리턴 타입을 보고 결정한다.
            // => List라면 selectList()를 호출하고,
            // => 그냥 일반객체라면 selectOne()을 호출하고,
            // => int라면 insert()/update()/delete()를 호출한다.
            if (retType == List.class) {
              // 리턴 타입이 List일 경우 목록을 select 문을 실행하는 것이다.
