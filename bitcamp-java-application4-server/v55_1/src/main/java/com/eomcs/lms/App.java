...
public class App implements Servlet {
  ...
  public void init(ServletConfig config) throws ServletException {
    ...
    // 이 객체가 작업을 수행하는데 필요한 자원들을 준비할 수 있도록 이 메소드를 호출한다.
  ...
  public void service(
    ...
    try {
      ...
      if (requestHandler != null) {
        ...
        out.println(
            "<html><body>"
            + "<h1>해당 명령을 처리 할 커맨드를 찾을 수 없습니다.</h1>"
            + "</body></html>");
  ...
  @Override
  public void destroy() {
    // 톰캣 서버가 종료되기 전에 이 객체가 준비한 자원들을 해제할 수 있도록 이 메소드를 호출한다.
    
  }
  
  @Override
  public String getServletInfo() {
    // 톰캣 서버가 관리자 페이지에 애플리케이션에 대해 간단한 소개를 출력하기 위해 이 메소드를 호출한다.
    // 즉, 이 메소드의 리턴 값을 관리자 페이지에 출력한다.
    return "수업관리 시스템";
  }
  
  @Override
  public ServletConfig getServletConfig() {
    // 이 객체를 실행하면서 이 객체의 실행 환경 정보를 알고 싶을 때 이 메소드를 호출한다.
    // 리턴값은 init()에서 받은 파라미터 값이다.
    // 따라서 init() 메소드에서 파라미터 값을 잘 보관해 둬야 한다.
    return this.config;
  }
